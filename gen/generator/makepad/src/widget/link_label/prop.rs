...
use gen_utils::{
    error::Errors,
    props_manul::{Background, Event, Font, Others, Position, Resource, Size, Text},
};
...
pub struct LinkLabelProps {
    ...
    pub open_in_place: Option<bool>,
}

impl DynProps for LinkLabelProps {
    fn prop_bind(
        ...
        match prop.name() {
            ...
            Resource::HREF => quote_prop(vec!["url"], &value),
            Resource::TARGET => quote_prop(vec!["open_in_place"], &value),
            _ => panic!("cannot match prop in BuiltIn label"),
        }
    }
}

impl StaticProps for LinkLabelProps {
    ...
    fn prop(&mut self, prop_name: &str, value: gen_parser::Value) -> () {
        let _ = match prop_name {
            ...
            // ----------------- walk -----------------
            Size::HEIGHT => self.height(&value, NodeType::LinkLabel),
            Size::WIDTH => self.width(&value, NodeType::LinkLabel),
            Position::ABS_POS => self.abs_pos(&value, NodeType::LinkLabel),
            Size::MARGIN => self.margin(&value, NodeType::LinkLabel),
            // ------------------- layout -----------------
            ...
            LINE_SPACING => self.line_spacing(&value, NodeType::LinkLabel),
            // ------------------- other ------------------
            Event::GRAB_KEY_FOCUS => self.grab_key_focus(&value),
            Text::TEXT => self.text(&value),
            Resource::HREF => self.url(&value),
            Resource::TARGET => self.open_in_place(&value),
            _ => {
...
impl ToToken for LinkLabelProps {
...
impl LinkLabelProps {
    fn url(&mut self, value: &Value) -> Result<(), Errors> {
        string_prop(value, |s| {
            self.url = Some(s.to_string());
        })
    }
    fn open_in_place(&mut self, value: &Value) -> Result<(), Errors> {
        bool_prop(value, |b| {
            self.open_in_place = Some(b);
        })
    }
    ...
    fn line_spacing(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::LinkLabel => self.check_layout().line_spacing(value),
        }
    }
    ...
    fn height(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::LinkLabel => self.check_walk().height(value),
        }
    }
    fn width(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::LinkLabel => self.check_walk().width(value),
        }
    }
    fn abs_pos(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::LinkLabel => self.check_walk().abs_pos(value),
        }
    }
    fn margin(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::LinkLabel => self.check_walk().margin(value),
        }
    }
...
impl Display for LinkLabelProps {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(url) = self.url.as_ref() {
            let _ = f.write_fmt(format_args!("url: \"{}\",", url));
        }
        if let Some(open_in_place) = self.open_in_place.as_ref() {
            let _ = f.write_fmt(format_args!("open_in_place: {},", open_in_place));
        }
        write!(f, "")
    }
}
