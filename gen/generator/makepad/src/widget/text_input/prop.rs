...
use crate::{
    prop::{
        builtin::{
            draw_color::DrawColor, draw_icon::DrawIcon, draw_text::DrawText, Align, Layout, Walk
        },
...
impl TextInputProps {
    ...
    fn check_draw_radio(&mut self) -> &mut DrawTextInput {
        if self.draw_radio.is_none() {
            self.draw_radio = Some(DrawTextInput::default());
        }
    ...
    fn curve(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::Outer => Err(Errors::PropConvertFail(format!(
                "Makepad TextInput has no outer curve! (icon_curve|curve)",
            ))),
        }
    }
    fn line_spacing(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            NodeType::Icon => Err(Errors::PropConvertFail(format!(
                "Makepad TextInput has no icon line_spacing! (label_line_spacing|line_spacing)",
            ))),
    ...
    fn draw_depth(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::Outer => Err(Errors::PropConvertFail(format!(
                "Makepad TextInput has no outer draw_depth! (icon_draw_depth|draw_depth)",
            ))),
        }
    }
    fn color(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::Outer => Err(Errors::PropConvertFail(format!(
                "Makepad TextInput has no outer color! (icon_color|color)",
            ))),
        }
    }
    ...
    fn brightness(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            ...
            NodeType::Outer => Err(Errors::PropConvertFail(format!(
                "Makepad TextInput has no outer brightness! (icon_brightness|brightness)",
            ))),
        }
    }
    ...
    fn align(&mut self, value: &Value, ty: NodeType) -> Result<(), Errors> {
        match ty {
            NodeType::Icon => Err(Errors::PropConvertFail(format!(
                "Makepad TextInput has no icon align! (icon_align|align)",
            ))),
            NodeType::Label => {
